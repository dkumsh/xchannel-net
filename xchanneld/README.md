...
The node-manager daemon for [xchannel-net](https://github.com/dkumsh/xchannel-net). One runs per
machine. It owns the decentralized registry, discovers channels across the mesh, and replicates
single-writer [xchannel](https://github.com/dkumsh/xchannel) logs between nodes.

This crate is **only** the program. The node manager itself is
[`xchannel-net`](https://crates.io/crates/xchannel-net), which depends on nothing but `xchannel`;
the split exists so that a command-line parser cannot reach anyone embedding the manager as a
library. Applications talk to the daemon through
[`xchannel-net-client`](https://crates.io/crates/xchannel-net-client) and then read and write purely
local memory-mapped logs.
...
xchanneld --help
xchanneld --seeds 10.0.0.5:7001
```

Every option is a flag **and** an environment variable, so `systemd`, `docker -e`, Kubernetes
`env:`, and the client's own auto-spawn all keep working unchanged. A misspelled *flag* is an error;
a misspelled *variable* cannot be, so the daemon warns at startup about any `XCHANNELD_*` it does
not recognise.

| Variable | Flag | Default | |
|---|---|---|---|
| `XCHANNELD_NODE_ID` | `--node-id` | generated on first start | Stable identity, 64 random bits kept in `<data_dir>/.node_id`. Set it only for deployments needing deterministic ids; a *configured* id is never discarded automatically, so resolving a duplicate is then yours to do. |
| `XCHANNELD_NODE_NAME` | `--node-name` | this host's name | Human-readable label, gossiped for display only. Never a key, never a tie-break. |
| `XCHANNELD_DATA_DIR` | `--data-dir` | `$HOME/.xchannel-net` | Channel files, replicas, the node's identity, and the client socket. One daemon per directory, enforced by a lock. Must be a **local** filesystem: channels are memory-mapped, and mmap coherence over NFS or SMB is not something this relies on. |
| `XCHANNELD_CLIENT_PATH` | `--client-path` | `<data_dir>/client.sock` | Local client plane. Must match what clients look for, so change both or neither. |
| `XCHANNELD_STREAM_ADDR` | `--stream-addr` | `127.0.0.1:7000` | Data plane — the address to **bind**. |
| `XCHANNELD_CONTROL_ADDR` | `--control-addr` | `127.0.0.1:7001` | Registry gossip and heartbeats — the address to **bind**. |
| `XCHANNELD_ADVERTISE_STREAM_ADDR` | `--advertise-stream-addr` | the bound address | What to tell peers, when it must differ from what was bound. |
| `XCHANNELD_ADVERTISE_CONTROL_ADDR` | `--advertise-control-addr` | the bound address | Same for the control plane. Required in practice when binding a wildcard: peers gossip whatever is advertised, and `0.0.0.0` is not something any of them can dial. **Must be this instance's own address** — it is what duplicate-`NodeId` detection compares, so two nodes advertising one value are as indistinguishable as two advertising a wildcard. A wildcard advertised value warns at startup; a shared routable one cannot be detected at all, by construction. |
| `XCHANNELD_SEEDS` | `--seeds` | — | Peer control addresses to form the mesh with. Comma-separated, or repeat the flag. An empty value means none. |
| `XCHANNELD_RECLAIM_AFTER_MS` | `--reclaim-after-ms` | `300000` | How long an owner must be unreachable *from this node* before an operator may reclaim its name. |
| `XCHANNELD_PROMOTED_TOPICS` | `--promoted-topics` | — | Topics given a merge thread of their own instead of the shared duty cycle. |
| `XCHANNELD_MUX_MAX_PARK_US` | `--mux-max-park-us` | `5000` | Cap on how long an idle duty cycle parks; `0` never parks. |
